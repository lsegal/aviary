# MCP Reference Index

These pages enumerate the currently registered MCP tools from `internal/mcp/tools.go`.

## Categories

- [Agent Tools](./agents)
- [Session Tools](./sessions)
- [Task And Job Tools](./tasks-and-jobs)
- [Browser And Channel Tools](./browser-and-channels)
- [Files And Notes Tools](./files-and-notes)
- [Auth Tools](./auth)
- [Server And Config Tools](./server-and-config)
- [Usage And Skills Tools](./usage-and-skills)

## Next Step For Each Page

Every page should grow to include:

1. The tool purpose.
2. The request shape.
3. Example calls.
4. Return payload shape.
5. Side effects, permissions, and failure modes.
