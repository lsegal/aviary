...
func buildToolSystemPrompt(agentName string, tools []ToolInfo, query string) string {
	...
	sb.WriteString("When asked to schedule a task or reminder, call task_schedule immediately using your own agent name. Use \"in\" for one-time reminders and \"schedule\" for recurring tasks. Prefer omitting task type unless you already have concrete script text; Aviary can promote deterministic prompt requests into script tasks automatically. Do not ask where output will appear — scheduled task output is captured in job logs.\n")
