	...
	<div class="mx-auto max-w-2xl px-4 py-12">
			...
			<div v-if="credMethod === 'oauth'" class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
				<template v-if="currentProvider?.id === 'anthropic'">
					...
					<div v-else class="space-y-4">
						...
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-left dark:border-gray-700 dark:bg-gray-800">
				...
				<template v-else-if="currentProvider?.id === 'openai'">
					<div v-if="!openAIUrl">
						<p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
							Sign in with your ChatGPT Plus or Pro account. We'll open OpenAI's consent page and also show the full URL here in case the browser opens on a different machine.
						</p>
						<p class="mb-4 text-xs text-gray-500 dark:text-gray-400">
							This uses the <code class="font-mono">openai-codex</code> provider and requires a ChatGPT Plus or Pro subscription.
						</p>
						<button
							type="button"
							:disabled="credSaving"
					...
					<div v-else class="space-y-4">
						<p class="text-sm text-gray-700 dark:text-gray-300">
							Open the authorization URL below on a machine that can reach the callback URL, finish sign-in there, then come back here and click complete.
						</p>
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-left dark:border-gray-700 dark:bg-gray-800">
							<p class="text-xs font-medium text-gray-700 dark:text-gray-300">Authorization page</p>
							<a
						...
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-left dark:border-gray-700 dark:bg-gray-800">
							...
							<p class="mt-1 break-all font-mono text-[11px] text-gray-700 dark:text-gray-300">{{ openAICallbackUrl }}</p>
						</div>
						<p
						...
						<button
							type="button"
							:disabled="credSaving || openAITimedOut"
							class="w-full rounded-lg bg-green-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-green-500 disabled:opacity-40"
				...
				<template v-else-if="currentProvider?.id === 'gemini'">
					<div v-if="!geminiUrl">
						<p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
							Sign in with your Google account. We'll open Google's consent page and also show the full URL here in case the browser opens on a different machine.
						</p>
						<button
							type="button"
							:disabled="credSaving"
					...
					<div v-else class="space-y-4">
						...
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-left dark:border-gray-700 dark:bg-gray-800">
							<p class="text-xs font-medium text-gray-700 dark:text-gray-300">Callback URL</p>
							<p class="mt-1 break-all font-mono text-[11px] text-gray-700 dark:text-gray-300">{{ geminiCallbackUrl }}</p>
						</div>
						<p
						...
						<button
							type="button"
							:disabled="credSaving || geminiTimedOut"
							class="w-full rounded-lg bg-blue-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-400 disabled:opacity-40"
		...
		<div v-else-if="step === 'done'" class="text-center">
			...
			<div class="flex items-center justify-center gap-3">
				...
				<router-link to="/chat" class="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-500">
					Start chatting ->
				</router-link>
			</div>
		</div>
