...
export function useLogs() {
  ...
  const filtered = computed(() => {
    ...
    return entries.value.filter((e) => {
      ...
      if (filterComponents.value.size > 0 && !filterComponents.value.has(e.component)) return false;
      if (text) {
        const haystack = `${e.component} ${e.msg} ${Object.entries(e.attrs ?? {}).map(([k, v]) => `${k}=${v}`).join(" ")}`.toLowerCase();
