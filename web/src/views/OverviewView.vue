<template>
  <AppLayout>
    ...
    <div v-else class="px-6 py-6">
      ...
      <div v-if="store.error" class="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900 dark:bg-red-950 dark:text-red-400">
        {{ store.error }}
      </div>
      ...
      <div class="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="flex flex-col rounded-xl border p-5" :class="healthCardClass">
          ...
          <div class="flex items-center gap-2">
            ...
            <svg v-else class="h-5 w-5 shrink-0" :class="healthTextClass" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
      ...
      <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-800">
          ...
          <div class="flex items-center gap-3">
            ...
            <button
              ...
              @click="store.recheck()"
            >
              <svg class="h-3 w-3" :class="{ 'animate-spin': store.loading }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
        ...
        <!-- All clear -->
        <div v-else-if="store.issues.length === 0"
          class="flex items-center gap-3 px-5 py-8"
        >
          <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/40">
        ...
        <div v-else>
          ...
          <div v-if="errorCount > 0">
            <div class="flex items-center gap-2 bg-red-50 px-5 py-2.5 dark:bg-red-950/20">
              <svg class="h-3.5 w-3.5 text-red-500 dark:text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
              </svg>
              <span class="text-xs font-semibold text-red-600 dark:text-red-400">{{ errorCount }} Error{{ errorCount !== 1 ? 's' : '' }}</span>
            </div>
          ...
          <div v-if="warnCount > 0">
            <div class="flex items-center gap-2 border-t border-gray-100 bg-yellow-50 px-5 py-2.5 dark:border-gray-800 dark:bg-yellow-950/20">
              <svg class="h-3.5 w-3.5 text-yellow-500 dark:text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9.303 3.376c.816 1.3.258 3.374-1.948 3.374H4.645c-2.206 0-2.764-2.074-1.948-3.374l6.657-10.748c1.083-1.75 3.51-1.75 4.593 0l6.656 10.748zM12 15.75h.007v.008H12v-.008z" />
              </svg>
              <span class="text-xs font-semibold text-yellow-600 dark:text-yellow-400">{{ warnCount }} Warning{{ warnCount !== 1 ? 's' : '' }}</span>
            </div>
            <ul class="divide-y divide-gray-50 dark:divide-gray-800/50">
